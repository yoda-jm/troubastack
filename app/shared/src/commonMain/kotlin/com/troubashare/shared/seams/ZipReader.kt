...
/**
 * Size-cap (zip-bomb) guard, portable. True if extracting an entry of [uncompressedSize] bytes on
 * top of [written] bytes already written would exceed [cap]. A negative [uncompressedSize] — a size
 * above 2 GiB that overflowed the 32-bit central-directory field — is treated as exceeding, so a
 * bomb can never masquerade as a "small" (negative) entry that slips under the cap. Extracted here
 * from the iOS `unpackBundle` so the decision is unit-testable off-device (Android streams its own
 * running check via `ZipInputStream`).
 */
internal fun exceedsSizeCap(written: Long, uncompressedSize: Int, cap: Long): Boolean {
    if (uncompressedSize < 0) return true
    return written + uncompressedSize.toLong() > cap
}
