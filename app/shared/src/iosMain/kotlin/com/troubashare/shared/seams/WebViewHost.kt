 ...
 * `WKScriptMessageHandler` (bridged so `window.TroubaShareShell.receive(json)` from bridge.ts keeps
 * working), shell→web via `evaluateJavaScript` calling `window.__troubashareShell.deliver`.
 *
...
actual class WebViewHost {
    ...
    private val navigationDelegate = object : NSObject(), WKNavigationDelegateProtocol {
        ...
        override fun webView(webView: WKWebView, didFinishNavigation: WKNavigation?) {
            ...
            postToStudio("""{"type":"hello","shell":"troubashare-ios"}""")
        }
    ...
    init {
        ...
        // Route bridge.ts's `window.TroubaShareShell.receive(json)` into the script-message handler,
        // matching the Android @JavascriptInterface surface so one bridge.ts serves both shells.
        val shim = "window.TroubaShareShell = { receive: function(j) { " +
    ...
    actual fun postToStudio(message: String) {
        view.evaluateJavaScript(
            "window.__troubashareShell && window.__troubashareShell.deliver(${jsQuote(message)})",
            null,
        )
    }
