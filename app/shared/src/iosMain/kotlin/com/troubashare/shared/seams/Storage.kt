...
actual class Storage {
    ...
    private companion object {
        const val SERVICE = "troubashare.secrets"
    }
}
...
actual fun unpackBundle(zipPath: String, destDir: String): UnpackResult {
    ...
    return try {
        for (entry in archive.entries) {
            ...
            // Size-cap guard before inflating (bomb protection).
            if (entry.uncompressedSize < 0 || written + entry.uncompressedSize > MAX_ARCHIVE_BYTES) {
                return UnpackResult.Failed("that file is too large to be a concert")
            }
