...
|---|---|
| ![The TroubaStudio editor: a score with freehand, shape and text annotations, per-member layers, realtime sync](docs/screenshots/studio-editor.png) | <img src="docs/screenshots/stage-page.png" alt="TroubaStage on Android: a baked page composited with its annotation overlays, pager and layer controls" width="260"> |
...
A tiny baked concert is committed at
[`docs/demo/demo-concert.tstage`](docs/demo/demo-concert.tstage) (10 KB, generated by
`core/cmd/mkbundle`), so you can present the app without running anything:
...
in airplane mode. Navigation (tap/swipe/song jump), fit modes, per-layer visibility and
the local “Role” picker all work offline; the screen stays awake and the system bars hide
while performing. Real bundles will come from the server-side bake
