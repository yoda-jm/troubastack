...
> ADR; "test" = there is (or must be) an automated check; "structure" = the repo layout makes
> the violation hard.
...
**Enforced.** spec + structure + test (codegen runs in CI; generated files are not edited).
...
**Enforced.** spec + review. *(Need divergent versions? Duplicate as a new song — see design doc.)*
...
**Enforced.** review + test (a GC pass must prove no live reference is dropped, across all layers).
...
**Enforced.** structure (bake = headless dry render) + review. See [design/03](design/03-rendering-and-ink.md).
...
| I8 | `web/ink`, native overlay parity test |
