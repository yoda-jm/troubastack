...
## Part E — BLOCKER (CONFIRMED, VLL phone repro): tapping a draw tool doesn't select it on touch

**Priority #1 of T66 — an editor where you can't select a tool on a phone is broken.** VLL pinned it exactly: **phone browser → tap a writing/draw tool → the tool is NOT selected and the secondary (style/ctx) toolbar never appears.** (My clean emulated taps couldn't reproduce it — a synthetic tap fires a clean click; a real finger with micro-movement inside a horizontally-scrollable row is read as a scroll gesture and the click is cancelled, so `setTool` never runs → `ctxShown = tool!=="select" || selected` stays false → no secondary bar. `.edit-canvas` touch-action is fine, styles.css:1050; the problem is the TOOLBAR eating the tap, not the canvas.)

- **Root cause:** the horizontally-scrollable toolbar (Part C single-scroll, and the T65 tool row when it overflows) consumes a finger-tap on a tool button as a scroll drag → the button's click never fires.
- **Fix:** the tap must win over the scroll. Set `touch-action: pan-x` on the scroll container (so a tap/vertical gesture still fires the button's click while horizontal drag scrolls), and/or a small movement threshold before the row treats a pointer as a scroll. The tool buttons MUST reliably select on a single finger tap anywhere in the row, including mid-scroll and under the fade.
- **e2e (real-touch-faithful):** tap each draw tool via the touchscreen and assert (a) it becomes active (aria-pressed) AND (b) the secondary/ctx toolbar renders AND (c) a finger draw then creates an object. A synthetic-clean tap is NOT sufficient — drive `touchscreen.tap`/pointer with a tiny move so the test actually exercises the scroll-vs-tap discrimination.
- **Landed-T65 exposure (flag):** on landed main (T65) the tool row also scrolls when it overflows; at ~390px the 8 tools fit (no overflow → no bug), so T65 bites only on narrower phones / when overflowing. T66's single-scroll bar overflows ALWAYS → always bites. If VLL's current phone is narrow enough that T65 already overflows, this is ALSO a live main bug — fix lands with T66 (which supersedes the tool-row anyway); no separate T65 hotfix unless VLL is blocked before T66 lands.
- Reviewer will re-test draw-tool selection on a real-touch path in the T66 build before GO.
