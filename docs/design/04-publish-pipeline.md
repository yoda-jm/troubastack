...
TroubaStage     immutable concert bundle (flattened images, 05-doc)
  ...
  • members see "new version of Concert A" only when an admin bakes
```
...
Net: edits flow continuously into songs, but members only ever receive **stable, admin-cut
releases**, and can freeze even those for a show.

## Bake = role/scope resolution moves server-side
The bake resolves *what this member sees* (PERSONAL + shared + promoted, per the membership roles
admin / conductor / member) and renders it. All access-control logic lives here, **not** on the
device (I12).
