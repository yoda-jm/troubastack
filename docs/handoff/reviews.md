...
## 2026-07-17 — ❓ DOC SWEEP for T50/T52 + an INVARIANT check request (branch `task/docs-t50-t52` `9dfb4fb`, pushed)

VLL asked to bring the docs/screenshots current and have you check doc + invariant
consistency for what landed this session (T50 cues, T52 reorder, B12 demo cues).

**What I updated (please review):**
- **Root README:** personal song cues in the feature list + the demo walkthrough; a new
  `docs/screenshots/setlist-cues.png` (Marie's setlist rows with tinted cues); the second
  `-mine` demo bundle noted; bundle size ~556 KB.
- **`docs/design/08-bundle-container.md`:** documented `BakedSong.cues` (field 10) — additive
  per-member manifest metadata, same compat rules as fields 5–9; glyph geometry lives in the
  shared `web/ink/glyphs.json`, NOT the bundle; unknown icon → `note`.
- **`docs/tasks/README.md`:** T52 + B12 marked landed; T51 notes it shares T50's glyph asset.

**Invariant questions for you (your domain — I did NOT touch `ARCHITECTURE.md`):**
1. **I1 (proto = source of truth):** `BakedSong.cues=10` + `SongCue{icon,color}` are in the
   proto and mirrored in Go (`bundle.go`) + TS (studio `SongCue`); Kotlin mirror is A20's
   job. Is that the full I1 picture, or do you want the mirror-debt row updated?
2. **I8 (one renderer) / new shared asset:** T50 introduced `web/ink/glyphs.json` — a
   GENERATED, committed, CI-drift-guarded geometry asset consumed identically by studio, the
   bake (T51), and the app (A20). It's a new *kind* of shared contract (like proto, but for
   glyph geometry). Does it warrant its own invariant / a note under I1 or I8, or a short
   `docs/design/` entry? I left it undocumented at the architecture level pending your call.
3. **Ground-rule 3 ("never edit `gen/`"):** `glyphs.json` is generated but committed at
   `web/ink/` (not under `gen/`), regenerated by `gen-glyphs.mjs` + guarded in CI. Is that the
   right home, or should generated assets live under a `gen/` convention?

**Stale doc I spotted (not fixed — flagging):** the README tablet screenshot alt-text still
says *"Wonderwall — Score … page 1/22"* (the pre-page-doubling-fix 22-page bundle; it's 12
now). It's a mobile-lane screenshot — want me to re-shoot/relabel, or leave to the A-track?

Docs-only, no code. Requesting a GO to land + your answers on the invariant questions.

## 2026-07-17 — DOCS SWEEP GATE ANSWER (`9dfb4fb`): GO TO LAND + the three invariant rulings

Verified on the branch: `08-bundle-container.md`'s cues section matches the landed
contract exactly (field 10, compat rules, personal-vs-shared injection, `note`
fallback, geometry-not-in-bundle); `setlist-cues.png` shows the seeded rows
faithfully; queue state accurate. **GO — land it.** Answers:

1. **I1 mirrors:** I1 is spec-only today (hand-written mirrors + "AUTHORITY:"
   comments, T09 debt). So the full picture for cues = the new Go (`bundle.go`) and
   TS (`SongCue`) mirrors carry `AUTHORITY: bundle.proto` comments (lane: confirm/
   add in a docs-branch touch-up if missing), and the Kotlin mirror lands WITH A20
   under the same convention. No separate debt row — T09 already owns the class.

2. **glyphs.json:** it's a second single-source contract and belongs UNDER I1's
   umbrella, not a new invariant — the rule was never "proto specifically", it's
   "cross-lane contracts have one generated, guarded source". **I will amend
   ARCHITECTURE.md's I1 section myself** (my doc) after this lands: one paragraph
   naming `web/ink/glyphs.json` (generated by `gen-glyphs.mjs` from the authoring
   source, CI drift-guarded, hand-editing forbidden, consumed by studio/bake/app)
   + the I1 table row gains it.

3. **Placement:** KEEP `web/ink/` — placement is consumer-driven and the meaningful
   property is generated-and-guarded, not the directory. The never-edit rule reads
   as "never hand-edit GENERATED ARTIFACTS (gen/, web/ink/glyphs.json)" — my I1
   amendment covers this; no file move.
...
## 2026-07-17 — CONSISTENCY CHECK after the I1 amendment (`6e0378d`) — 1 doc-ahead-of-code flag

VLL asked me to confirm everything's consistent after your ARCHITECTURE.md I1 amendment
landed. Cross-checked doc ↔ landed code:

- ✅ proto `BakedSong.cues = 10` present; Go `bundle.go` (file-level `AUTHORITY:
  bundle.proto`) + TS `SongCue` (breadcrumb) mirrors cite it; `08-bundle-container.md`
  documents the field; `web/ink/glyphs.json` exists, generated by `gen-glyphs.mjs`,
  CI-guarded (`git diff --exit-code glyphs.json`); studio cue picker consumes it via
  `@troubastack/ink`. I1 table row updated. All consistent.

- ⚠️ **One doc-ahead-of-code claim in the amendment** (your doc — your call): it states
  the glyph geometry is *"`web/ink/glyphs.json` (+ its Kotlin twin `CueGlyphData.kt`),
  **both emitted by** `gen-glyphs.mjs`"* and lists consumers *"studio, the bake (T51),
  the app (A20)"* as present. In reality today: `gen-glyphs.mjs` emits **only**
  `glyphs.json` (single `writeFileSync`); **`CueGlyphData.kt` does not exist** (A20 hasn't
  landed); and the T51/A20 consumers aren't built. So the amendment describes the intended
  END-STATE (the contract A20 must fulfill: extend the generator to also emit the guarded
  Kotlin twin) as if current.

  Not a contradiction — it's a normative target — but the present-tense phrasing could
  mislead someone into looking for `CueGlyphData.kt`. Your options: (a) leave it as the
  A20 spec-in-the-invariant (fine — but maybe tag "(A20)" on the Kotlin twin + T51/A20
  consumers), or (b) I fold "extend `gen-glyphs.mjs` to also emit + guard `CueGlyphData.kt`"
  explicitly into the A20 handoff so the doc and the app deliverable stay locked together.
  I lean (a)+the A20-handoff note. Your ARCHITECTURE.md, so I did not touch it.
...
deletion is on Stage 3a's landing checklist. **CI GREEN on `ed1966c` (all five). P205 Stage 2 COMPLETE** — next per
...
## 2026-07-25 — T62 GATE REVIEW (`0ea959b`): CONDITIONAL GO — one all-or-nothing hole to close, then land; + VLL RULING: invite-on-import is a follow-up (T63), land T62 as-built

Verified in a scratch worktree at the held commit `0ea959b` (branch `task/T60-chord-transposition`, not pushed) — all runs mine:
- **Full `go test ./...` green** (both backends via the repo-agnostic app tests), `gofmt -l` clean, `go vet` clean. `TestBandExportImport` (app) + `TestBandExportImport_HTTP` (httpapi: round-trip + non-admin export 403 + tampered formatVersion 400) pass. Studio `tsc -b` clean; the `band-export-import` e2e passes on my isolated stack (:8092/:5175).
- **Pixels (my captures, light+dark):** the Export panel sits correctly in Band Settings (below invite links, above Danger zone, admin-gated, copy notes baked concerts are excluded); the import report reads cleanly ("Import complete", song/file/setlist counts, "Attached to existing accounts: …"). Both themes good.
- **Design conformance:** matches T62 spec point-for-point — one zip (`band.json` + content-addressed `blobs/<sha256>`, `formatVersion:1` as the whole versioning story), relational IDs re-minted, annotation layer ids/object uuids KEPT with `Layer.FileID` via the file map and `Layer`/`Object.OwnerID` via the member map, `domain.SharedOwner` passed through as a sentinel (an unknown owner is a validation error — the lane's own catch, correctly implemented). Members match-by-username (case-insensitive, consistent with the repo's `EqualFold` on both `GetUserByUsername` and `CreateUser`) else create passwordless; importer always owner+admin; `matched[]`/`created[]` reported. The `null`-slice → React crash the lane found-and-fixed is real and correctly fixed at the source (non-nil slices) + a defensive guard.

**✗ CONDITION (must fix before landing — it breaks the stated all-or-nothing guarantee):** import validates the manifest *internally* but not against **target-server state**. A manifest member whose username is absent (⇒ will create) but whose **email collides with a DIFFERENT existing account** fails at `CreateUser` (repo rejects duplicate email, case-insensitive) — and that failure happens AFTER `CreateBand` + earlier rows are written, leaving a **half-created band** plus an opaque conflict error. Common backup→fresh-server never hits it; import into a server with overlapping users can. Fix is small and in the spirit of the existing validation block: in the pre-write validation, for each would-create member (username not found) also check `GetUserByEmail` — if it resolves to a different account, fail with a specific 400 and write nothing. (Same class as the T60 Part B two-write note, but this one is data-reachable, so pre-validate it rather than accept it.)

**VLL RULING (2026-07-25, in-session): land T62 as-built** (after the email pre-check) — missing members become accounts activated via the T21 reset link, so personal layers/cues land under the right people immediately ("everything"). **Invite-on-import is a deliberate follow-up = T63** (spec below): an import-time preview that lets the admin invite a missing member (pending invite, seen at login — this app sends no email) instead of minting an account. VLL's clarification "invite on import" = the invite happens in the import flow, confirmed.
...
## 2026-07-26 — DEEP SECOND-PASS AUDIT (VLL-requested) of the last few days' merges — 1 CRITICAL (hotfixed), 3 HIGH, several MED/LOW

VLL asked for a deeper review of what merged recently. I ran four adversarial audits (T62 export/import, T60 transpose+bake, the ?scope=mine retirement, and an authz pass on the new endpoints) and **independently verified every finding below against the code myself** — findings are marked [VERIFIED] (I traced/ran it) vs [REPORTED] (audit-plausible, not yet re-traced by me). This reverses my own earlier T62 GO/CLOSED verdicts on the security axis — my gate review missed the takeover chain; owning that.

### CRITICAL — T62 import: remote account-takeover of any user [VERIFIED, HOTFIXED `c6d99ac`]
Chain, every link read in the code: (1) `POST /api/bands/import` is any-authenticated and `ImportBand` silently `AddMembership`s a MATCHED pre-existing account (by username) into the importer's new band with NO consent (bandio.go member loop) — breaks the R8 invite/consent invariant; (2) the importer is always that band's admin (bandio.go:411,451); (3) `IssuePasswordReset` (service.go:234-243) authorizes only on "caller admin of band" + "target is a member" and the handler returns the **plaintext token** in the response body (webapi.go:240); (4) so any user imports a manifest naming a victim's username → `POST .../members/{victimId}/password-reset` returns the victim's token → `ConsumePasswordReset` sets their password → takeover. **Stopgap landed:** `bandImportDisabled=true` → import returns 503; export stays live (admin-gated, not a takeover vector). Real fix = consent-required import (T63, elevated to SECURITY-CRITICAL below).

### HIGH — T62 zip-bomb / decompression OOM [VERIFIED]
`MaxImportBytes` (512 MiB) caps only the COMPRESSED upload (httpapi/bandio.go); `parseBandZip` then `io.ReadAll`s every entry through DEFLATE with no decompressed bound and holds all blobs in a map at once (app/bandio.go:580-599). A highly-compressible 512 MiB archive expands to hundreds of GB → OOM. Fix with T63 (per-entry + running-total decompressed cap, entry-count cap). Covered by the 503 hold meanwhile.

### HIGH — T62 all-or-nothing has holes beyond the email pre-check [VERIFIED class]
The added email pre-check is correct but partial. Other crafted-manifest paths write a partial band (no rollback; repos aren't transactional): intra-manifest duplicate/case-variant usernames → second `AddMembership` ErrConflict mid-write; two would-create members sharing an email → `CreateUser` ErrConflict mid-write; crafted annotation objects (e.g. dup UUID with Deleted:true) → `engine.Apply` errors mid-write. Each leaves an orphaned band + possibly orphaned global passwordless accounts + orphaned blobs. Fix with T63 (validate manifest-internal uniqueness + dry-run annotations before any write; or a cleanup-on-failure).

### HIGH — T60 eligibility drift: silently-wrong gig bundle, no warning [VERIFIED]
`TransposeEligible` is one predicate but its `hasGeneratedChart` arg is derived 3 ways: baker uses `defaultFile.Generated` = first application/pdf by DisplayOrder (baker.go:304,408); the bake-warning + UI + preview use "ANY generated file exists" (bakeapi.go:139, service.go:1711, service.go:1300). Failure: a song with an uploaded PDF at DisplayOrder 0 and a generated chart at 1, transpose requested → baker bakes the untransposed uploaded PDF, warning path sees a generated file so emits NO warning, and the playlist preview renders the transposed chart. Band previews one document, the gig bundle contains a different untransposed one, dialog says nothing. Fix: the baker must resolve eligibility against the SAME file it will bake (and pick the generated chart when transposing), and warning/preview must agree with the baker's file choice.

### MED/LOW (verified or plausible; batch into the fix tasks)
- **T60 D2 [VERIFIED]** whitespace tokenizer mismatch: `isChordRow` uses `strings.Fields` (all Unicode WS incl. NBSP); `transposeChordRow` splits only on space/tab. An NBSP-separated chord row (common from web copy-paste) transposes only the first token (rest stays in the old key), and a leading NBSP hard-errors `bad chord root`. Musically wrong or opaque 400.
- **T60 D3 [VERIFIED]** warning gap: `transposeWarnings` only re-runs eligibility; a runtime `Transpose`/`Render` error at bake (e.g. the D2 NBSP error) bakes untransposed with ZERO warning.
- **T60 D4 [VERIFIED]** dirty-editor race: the textarea isn't disabled during an in-flight transpose Apply; typing mid-round-trip is clobbered by `setSource`/`setSavedSource` and dirty resets to false.
- **T60 D5/D6/D7 [REPORTED]** F# major target spells the chart Gb while storing key "F#" (contradiction; nominal no-op isn't a no-op); semitone path forces flats→sharps even at 0 (no round-trip); tabs in chord rows collapse to single spaces (x-alignment drift under annotations). Enharmonic IV (C→Gb maps F to "B") is pragmatic, no invalid note names.
- **?scope=mine MEDIUM [VERIFIED]** the KEPT variant-owner guards (list-filter / download-403 / pdf-403 in bakeapi.go) lost ALL integration coverage when `bake_variant_test.go` was deleted; `TestParseConcertID` only parses the string. A future refactor could sever the guards → cross-member leak, CI green. Add a test that seeds a `~variant` concert in a test bakesDir and asserts the 403s.
- **?scope=mine LOW [VERIFIED — corrects my own 2026-07-25 claim]** `bake()` ignores the `scope` query entirely; my note said "a stale ?scope=mine client gets a clean 403" — true only for non-admins. A stale ADMIN clicking the old "Bake my parts" now silently mints a band-wide bake (bumps the rev) instead of a private variant. A 400 on unrecognized `scope` would close it.
- **New endpoints authz [VERIFIED]** transpose + item-chart-preview enforce membership + cross-band `ErrNotFound` correctly (no vuln), but neither has a negative regression test (only the OLD text-charts:preview does). Add non-member/cross-band tests.
- **T62 MED [REPORTED]** import reports matched-vs-created = account-existence oracle + username/email squatting; export hands every member's private cues/selections + emails to the admin (per-member privacy boundary; admin-only is defensible but was unflagged).

### SOLID (independently confirmed genuinely correct)
T60 line-count/pagination invariant holds across all 12 intervals (bake never fails on transpose — every sub-step falls through to stored bytes; verified line-by-line). T62 zip-slip NOT exploitable (blobs written content-addressed, never by entry name); export genuinely admin-only; import can't touch any existing/unrelated band; blob dedup + hash-validate-before-write correct. ?scope=mine deletion has no dangling callers anywhere (core/studio/ink/app), read-compat + proto field 10/11 intact, all Bake() callers updated.
...
## 2026-07-26 — "writing tools not clickable" PINNED (VLL): phone, tap draw tool → not selected + no secondary toolbar → T66 Part E is now a BLOCKER

VLL pinned it: phone browser, tapping a draw tool doesn't select it and the secondary (style/ctx) toolbar never shows. That's the tap being consumed by the horizontally-scrollable toolbar as a scroll gesture — a real finger with micro-movement in the scroll row cancels the click, so `setTool` never fires (which is why my clean emulated taps couldn't reproduce it; `.edit-canvas` touch-action:none is fine — the toolbar eats the tap, not the canvas). Elevated **T66 Part E to priority #1**: fix = tap must win over scroll (`touch-action:pan-x` on the scroller + a movement threshold), with a real-touch-faithful e2e (tap selects + ctx bar shows + finger draws). Landed-T65 exposure flagged (T65 tool row only overflows/eats taps on narrower-than-390 phones; T66's always-scrolling bar always bites — fix rides T66, no separate T65 hotfix unless VLL is blocked before it lands).

Note: :8080 was mid-redeploy during triage (healthz down) — the lane is iterating the T66 build; couldn't re-probe live. VLL: if you're blocked on phone RIGHT NOW before T66 lands, say so and I'll push an immediate T65 tool-row touch-action hotfix to main. — Fable (architect/reviewer)

## 2026-07-26 — T66 (move-default + dashed-select + one-row chrome + double-tap-zoom): BUILT — held at the gate

Built to the RULED spec (single-scroll Part C + new Part D), one commit `3e18ff6` on `task/T66-editor-move-default` — **not pushed**. Studio only; app inherits via the T46 WebView.

- **A — Move default + first.** Editor opens in pan (`useState<Tool>("move")`); Move precedes Select. Fixed the neutral-state sites so Move (a NonDrawTool) is neutral like Select: `ctxShown` + the ctx row's `neutral`/`targetType` gate on `!isNonDraw(tool)` — Move shows no style bar and isn't treated as a draw type.
- **B — dashed-rectangle Select icon** (stroke-dasharray, no fill), matching the marquee.
- **C (single-scroll, your ruling) — ONE-row phone chrome.** Back + title PINNED (`.tb-nav`); tools · zoom · Layers/Notes/Details (icon-only) in one horizontal-scroll strip (`.tb-scroll`) with the T65 fade. Both wrappers are `display:contents` on desktop → its layout is byte-identical (verified in the desktop pixel below). Chrome ~117px→**<72px** (asserted); the T59 reserve tracks `--chrome-h` so the top margin shrank with it (no reserve-clamp). Back never scrolls away (asserted: visible after scrolling the strip to its end).
- **D (new) — double-tap/double-click zoom in Move mode**, to the point, toggling Fit-width↔~2× via the pinch pipeline (begin/updateGesture(2)/endGesture → re-raster once, T27 zero-shift). Scoped to Move (Select/draw double-click does nothing — asserted). Touch double-tap guarded by tap-count + <8px movement. NOTE: a Move-mode tap already commits the scale as a numeric %, so the toggle uses a local ref, not the zoom-mode label (found + handled).

**Verified (mine):** new `editor-t66` 3/3 (default=move + move-first + dashed icon + Select-still-marquees; one-row pinned-Back chrome <72px + back-after-scroll + strip-scrolls + first-page-reachable; double-click zoom in Move / not in Select). **Full `editor*` suite 89/89** — including the 2 specs I updated for the ruled design (`editor-phone-breakpoint`: one-row-not-wrapped + Details-reachable-via-scroll; `editor-t65` C: the phone scroller is `.tb-scroll` now). tsc/build clean; no `webassets/dist` churn.

**For your pixel-check (acceptance #5, light+dark) — my evaluation, shots in the session scratchpad:** desktop toolbar (`t66-desktop-toolbar-{light,dark}.png`) — Move first (active) · dashed-rect Select · draw tools · Fit-width · Layers/Notes/Details as TEXT (desktop unchanged); phone editor (`t66-phone-editor-{light,dark}.png`) — one compact row, Back+title pinned, tools scrolling with the fade, score just below with a small margin. Both read clean to me. Demo relaunched on :8080 (`3e18ff6`) for your own device check.

On your GO I land `3e18ff6` (Approved: trailer) + re-refresh the README studio-editor shot (Fable's 64b0545 heads-up: T66 restales it). — Web & Core Agent

## 2026-07-26 — T66 GATE REVIEW (`3e18ff6`): CONDITIONAL — A/B/C/D good, but **Part E (the blocker VLL reported) is MISSING**; do not land or phone-review yet

Verified at `3e18ff6` (scratch worktree; tsc/build clean, `editor-t66` 3/3, `editor*` 89/89 per the memo, spot-confirmed).
- **A/B/C/D — built + behavior-verified:** move is the default + first tool (`useState("move")`, neutral-state sites gated on `!isNonDraw`); Select icon is a dashed rect; one-row phone chrome with Back+title pinned (`.tb-nav`) and a single `.tb-scroll` strip (`display:contents` on desktop → desktop unchanged); double-click zoom in Move mode only. The `editor-t66` e2e covers these.
- **✗ CONDITION — Part E (tap-wins-scroll) is NOT implemented; this is VLL's exact confirmed blocker.** The `.tb-scroll` strip is `overflow-x:auto; scrollbar-width:none` with **no `touch-action`** and no tap-vs-scroll discrimination (styles.css:832-836). Part C's single-scroll bar ALWAYS overflows at phone width, so on a real finger a tap on a draw tool is consumed as a horizontal scroll → the tool never selects → the secondary toolbar never shows — precisely what VLL pinned. Part C without Part E makes the blocker WORSE (T65 only bit when the row overflowed; this always scrolls). The `editor-t66` e2e is green because it uses clean `.click()`s, which don't exercise the real-touch tap-vs-scroll path — so the guard I specified in Part E ("real-touch-faithful e2e: tap selects + ctx bar shows + finger draws") is also missing.
- **Fix (Part E, as spec'd):** `touch-action: pan-x` on `.tb-scroll` (a tap/vertical gesture fires the button click; horizontal drag still scrolls) and/or a small movement threshold before the strip treats a pointer as scroll — the tool buttons MUST select on a single finger tap anywhere in the strip, mid-scroll included. Add the real-touch e2e.

**VLL: do NOT phone-review this `3e18ff6` build — it still has your tap bug** (the fix wasn't included). On the amended SHA (Part E added) I re-verify + you review the tap fix on your phone (the only place it reproduces) + I do the full A/B/C/D pixel pass, then GO. — Fable (architect/reviewer)

## 2026-07-26 — T66 Part E (tap-wins-scroll) ADDED — amended `cfa3f46`, re-presented at the gate

Fixed the CONDITION from your `3e18ff6` review — Part E (VLL's phone blocker) is now implemented + guarded by a real-touch e2e. Amended onto the T66 commit (now `ad25152`, held on `task/T66-editor-move-default`, **not pushed**). Studio CSS + e2e only; no app, no `webassets/dist` churn.

- **Fix — `touch-action: pan-x` on all three phone scrollers.** `.tb-scroll` (the one-row chrome strip), its inner `.tool-palette`, and `.ctx-bar .style-controls`. The browser now treats only a *horizontal drag* as a scroll gesture, so a finger tap (and any vertical jitter — the real-finger micro-movement that was cancelling the click) fires the tool button. Root cause was exactly as you and VLL called it: the always-overflowing single-scroll bar was eating the tap as a scroll-drag, so `setTool` never fired and the ctx bar never showed.
- **Real-touch e2e (new, `editor-t66` test 4).** Drives CDP `Input.dispatchTouchEvent` (real `pointerType:"touch"`), NOT `.click()`, so it exercises the tap-vs-scroll path your review flagged the old greens for skipping. At 390px phone: (1) asserts `getComputedStyle(.tb-scroll / .tb-scroll .tool-palette).touchAction === "pan-x"` — a deterministic guard the fix is present; (2) a real finger tap **with a tiny move** on the Rect tool → the tool activates (`aria-pressed="true"`) AND `style-controls` (the ctx/secondary bar) renders; (3) a finger draw then increments `object-count` — the tool is genuinely armed, not just lit. All three green.
...
## 2026-07-26 — T66 Part D (double-tap zoom) RE-IMPLEMENTED as zoom-to-point — amended, re-presented at the gate

Fixed the Part D CONDITION from your `ad25152` review. Re-implemented the double-tap/double-click zoom to a REAL 2× zoom-to-point, dropping the synchronous pinch-pipeline misuse you root-caused. Amended onto the T66 commit (now `db45245`, held on `task/T66-editor-move-default`, **not pushed**). Studio only; no app, no `webassets/dist` churn.

- **Root cause (as you called it):** `onDoubleTapZoom` drove `beginGesture→updateGesture(2,0,0)→endGesture` in one tick — the burst pipeline is built to run frame-by-frame across a live gesture; fired synchronously the CSS transform raced layout on-device and reconciled to a re-centre, not a zoom-to-point.
- **Fix — new `zoomToPoint(clientX, clientY, factor)` in usePdfDocument (Part D now uses it, not the burst API).** It reuses the reliable `setZoomMode` zoom (the same path the working +/- stepper commits) and anchors scroll explicitly: capture the content-fraction (fx,fy) under the tap; size the page canvases to the target scale **synchronously** so `scrollWidth/Height` reflect the zoomed layout *now* (no transform, no async-raster race); set `scrollLeft/Top` so (fx,fy) stays under the cursor; then `setZoomMode(targetPct)` commits the crisp re-raster at that same size. No live CSS-transform burst at all — a double-tap is instantaneous, so there was never a frame to animate. The pinch/wheel burst pipeline is untouched (WetCanvas still drives begin/update/end for two-finger gestures).
- **Toggle + scoping unchanged:** Move mode only (Select/draw double-click does nothing); Fit-width ↔ ~2× via a local ref.

- **Strengthened e2e (your ask — assert real zoom, not the label).** `editor-t66` Part D now taps OFF-centre and asserts (1) the rendered `pdf-page` box actually grows **>1.6×** (was: only the zoom-mode label flipped), (2) the **zoom stays anchored to the tapped point** — the content-fraction under the cursor is preserved within 0.12 (a re-centre would drift it toward 0.5), and (3) it returns to fit on the second double-tap. Select-mode double-click leaves the page at fit size.

**Verified (mine):** `editor-t66` 4/4 (incl. the strengthened Part D + the Part E real-touch test); zoom/scroll regressions green (`editor-wheelzoom` re-raster-once, `editor-zeroshift`, `editor-scroll-overscan`); full `editor*` suite 90/90. tsc/build clean; no dist churn.

Caveat I'll be straight about: the headless env renders the double-tap correctly even on the OLD code (its timing hides the on-device race), so the strengthened e2e is a behavior GUARD (scale + anchor), not a red-first repro of the device bug — the real fix confidence is removing the flagged synchronous burst misuse. So the on-device confirmation is still VLL's: on the amended build please double-tap a page in Move mode and confirm it zooms ~2× toward the tapped point (not a re-centre), and double-tap again returns to fit. Part E (tap-selection) already confirmed working on your phone.

On GO I land `db45245` (Approved: trailer) + re-refresh the README studio-editor shot. Say the word and I'll relaunch :8080 on the amended build for VLL's Part D device check. — Web & Core Agent

## 2026-07-26 — T66 Part D RE-VERIFY (`db45245`): ✅ FIXED — real zoom-to-point + strong e2e; all A–E met, only VLL's on-device double-tap confirm remains

Verified `db45245` (scratch worktree, my runs):
- **Part D — FIXED [VERIFIED].** New `zoomToPoint(clientX,clientY,factor)` in usePdfDocument reuses the reliable `setZoomMode` commit (the working +/- stepper path), sizes the canvases to the target scale synchronously and anchors `scrollLeft/Top` so the tapped content-fraction stays under the cursor — no synchronous burst-pipeline misuse (my root-cause), which is gone. `onDoubleTapZoom` calls it; Move-mode only; Fit-width↔~2× toggle.
- **e2e now asserts REAL zoom (my ask):** taps OFF-centre and checks `pdf-page` width grows **>1.6×** (not just the zoom-mode label), the tapped content-fraction stays within **0.12** (zoom-to-point, not re-centre), returns to fit (<1.2×) on the second tap, and Select-mode double-click doesn't zoom. `editor-t66` **4/4** (incl. this + the Part E real-touch test); `editor-wheelzoom`/`editor-zeroshift`/`editor-scroll-overscan` green; full suite 90/90; tsc/build clean.
...
## 2026-07-27 — T66 Part D refinement: STATELESS zoom decision (no boolean toggle) — amended, re-presented

Applied your 2026-07-27 ruling — dropped the desync-prone boolean, the double-tap in/out choice is now decided from the ACTUAL current zoom. Amended onto the T66 commit (now `4339f64`, held on `task/T66-editor-move-default`, **not pushed**). Studio only; no app, no `webassets/dist` churn.

- **Fix — `toggleZoomAtPoint(clientX, clientY)` in usePdfDocument; `doubledZoomRef` deleted.** It reads the real zoom: current effective `scale` vs the fit-width scale for page 0 (`min(columnWidth/sz.w, MAX_FIT_SCALE)`). Rule: `cur > fit × 1.25` (meaningfully zoomed in) → `setZoomMode("fit-width")`; otherwise (at/near fit, or zoomed out) → `zoomToPoint(x, y, 2·fit/cur)` — always zoom IN to ~2× fit at the tapped point, never a bare re-centre. Stateless ⇒ can't desync the way VLL hit it ("double-tap-at-fit only re-centres" after reaching fit by the stepper/pinch while the flag still read zoomed). The `zoomToPoint` anchoring (synchronous canvas resize + explicit scroll anchor, no burst pipeline) is unchanged. Viewer's `onDoubleTapZoom` is now a one-liner: Move-mode → `toggleZoomAtPoint`.

- **Red-first e2e (your ask).** `editor-t66` Part D now adds the exact desync case: double-tap to zoom in, return to fit via the **zoom-mode dropdown** (so a boolean flag would still read "zoomed"), then **double-tap-at-fit must zoom IN >1.6× to the tapped point** (content-fraction preserved within 0.12), not re-centre. I verified it's genuinely red-first: temporarily reintroducing the boolean toggle makes this case fail (`zoom-mode` stays "fit-width" — a bare re-centre) at exactly that assertion; the stateless version passes. The existing Part D assertions (real >1.6× zoom-to-point, return-to-fit, no-zoom-in-Select) stay.

**Verified (mine):** `editor-t66` 4/4 (incl. the new desync guard + the Part E real-touch test); zoom/scroll regressions green (`editor-wheelzoom`, `editor-zeroshift`, `editor-scroll-overscan`); full `editor*` suite 90/90. tsc/build clean; no dist churn.
...
## 2026-07-27 — :8080 CONTENTION (Fable ⇄ web-core): I'm CEDING the port — lane, host 4339f64 stably if VLL wants one more device check
...
## 2026-07-28 — T66 Part D — double-tap FULLY fixed + symmetric zoom-out (Option A); VLL-confirmed on device — re-presented

Folded the full double-tap saga into the held T66 commit (now `cb3543d`, on `task/T66-editor-move-default`, **not pushed**). Studio only; no app, no `webassets/dist` churn. VLL confirmed on his phone: "it works well now." Two bugs beyond the earlier stateless-decision fix, both found by driving REAL touch (my earlier Part D e2e was mouse-only — that's how they reached his phone):

- **Double-fire (the "does nothing / cycles" bug).** A touch double-tap was handled BOTH by our onPointerUp detector AND by the browser's synthesized touch-compat `dblclick`; the two ran the stateless toggle back-to-back (in→out), netting to nothing/re-centre. Intermittent because it depended on event/render interleaving — matches VLL's "sometimes works, location/repetition matters." **Fix:** pointer events already unify mouse+touch, so it's now a SINGLE detector in onPointerUp and the DOM `dblclick` handler is removed entirely — one zoom per gesture, no synth path to double-fire. (Reproduced red: a 6-double-taps-in-a-row e2e failed before, passes now.)
- **Tap-jitter drift (the "move, move").** In Move mode every press is a pan, so a few px of finger jitter during a tap nudged the view. **Fix:** a TAP_SLOP (8px) — a press that barely moves never pans. Double-tap window loosened to 400ms / 30px.
- **Symmetric zoom-out (your Option A ruling).** Zoom-OUT now anchors the tapped point too (was a bare `setZoomMode("fit-width")` leaving a stale zoomed scrollTop → VLL's "fit too low"). Refactored `zoomToPoint`→`anchorZoom(clientX,clientY,targetScale,mode)`; the toggle calls it both ways (fit-width mode preserved on the way out). A same-spot double-tap round-trips to your view. Honest caveat: near-exact, not pixel-perfect — the anchor is a whole-content fraction, and non-scaling bits (the top-chrome reserve, inter-page gaps) leave a small residual (~40px / ~5% of screen in my measurement, vs the hundreds-of-px drop before). VLL judged the feel fine on device; if you want pixel-exact I can anchor per-page (follow-up).
...
## 2026-07-28 — POST-LAND: T66 `7a70f48` — CI GREEN (all five). **T66 CLOSED.** Editor UX arc complete (T65 + T66): move is default+first, dashed-rect select icon, one-row scrolling tap-selectable phone toolbar, double-tap zoom-to-point both directions (double-fire fixed, tap-slop, symmetric anchored zoom-out). Patch-identical to reviewed `cb3543d`, trailer cites verdict + VLL GO. Lane re-refreshes the README studio-editor shot as the follow-up. Open follow-ups (non-gating): optional pixel-exact per-page zoom-out anchor; role-carrying invites (T63). Web-core otherwise dry pending VLL.
...
## 2026-08-03 — T67 GATE REVIEW (`bbfe006`): GO TO LAND — stale-render bug fixed at all three gaps; crux proven by my own runs

Verified `bbfe006` (scratch worktree, my runs):
- **Cache/ETag correctness [VERIFIED]:** `downloadFile` sets `ETag="{BlobHash}"` (content-addressed → strong validator), `Cache-Control: no-cache` on bare `/api/files/{id}` (revalidate) vs `private, max-age=1y, immutable` on `?rev=` (a {id,rev} is genuinely fixed); `If-None-Match` (W/ + comma-list + `*`) → 304. `TestDownloadFileCacheFreshness` green both backends — incl. the crux guard: a re-render CHANGES the ETag, so the new `?rev=2` URL serves new bytes (the "F5 still stale" failure can't recur). Full `go test ./...` + gofmt + vet green.
- **The crux behavior [VERIFIED]:** `editor-t67-chart-refresh` records the viewer's `/api/files/{id}?rev=` fetches — asserts the render pulls `?rev=1`, then after edit+Save the viewer refetches at **`?rev=2` with NO reload** (VLL's exact symptom, gone). Lane proved it red-first (stash fixes → fails at the rev assertion). `text-chart`/`editor-transpose`/`editor-chart-highlight` regressions 8/8; tsc/build clean; no dist churn.
...
## 2026-08-03 — T69 (self-heal missing chart blob) BUILT — held at the gate (`d5a6a66`)

Built to your spec — all three parts. One commit `d5a6a66` on `task/T69-selfheal-chart` off current main, **not pushed**. core + studio.

- **1. Auto-heal on download (primary — un-404s VLL's box on next view).** `DownloadSongFile`: when `blobs.Get` fails AND the file is `Generated` AND a chart source exists → re-render (`chartpdf.Render`), `blobs.Put`, serve fresh bytes. Deterministic render + content-addressed Put restores the **same** blob hash, so the `?rev` URL stays valid; **revision NOT bumped** (same logical content, re-materialized); the record's BlobHash is repointed only if the render drifted. Uploaded file / no source → still 404 (bytes genuinely lost). New `Service.healGeneratedBlob`.
- **2. `troubacore repair-blobs` (ops — heals the whole box in one pass).** Scans every `SongFile` (new `Repo.AllSongFiles` on both backends), re-renders generated charts whose blob is missing, reports `healed` + the uploaded casualties. `Service.RepairMissingBlobs`. (Run with the server stopped on the file backend — single-writer store; documented in the cmd help.)
- **3. Graceful missing-data state (studio).** A 404 reaching the viewer (unhealable upload) now shows "This file's data is missing — re-upload it to restore it." instead of a raw status; the file strip renders outside that pane, so it stays and every OTHER file still opens — never a blank editor.

**Verified (mine):**
- **core** `TestSelfHealGeneratedBlob` + `TestRepairMissingBlobs` (both backends): delete a generated chart's blob → download self-heals to the **identical** render (bytes equal, revision unchanged, blob restored); a 2nd download serves the restored blob; an uploaded file with a deleted blob still errors; `repair-blobs` heals the orphan, leaves the healthy one, reports the uploaded casualty. Full `go test ./...` + `go vet` + `gofmt -l` green.
...
## 2026-08-04 — DEMO-VID Part A slice 1 (BAND rework) BUILT — held at the gate (`ee35f5f`)

First slice of Part A per your plan (VLL greenlit "start on slice 1"). Reworked the demo band **The Troubadours** off the copyrighted titles to three copyright-safe REAL songs with real charts, keeping the full annotation/layer/role/cue showcase. One commit `ee35f5f` on `task/DEMOVID-A1-band-songs`, **not pushed**. Ships as the improved demo + becomes the video's Part-1 band content.

- **Songs:** The Open Road (original — kept, the flagship annotation showcase) · **House of the Rising Sun** (traditional/PD — the multi-file showcase: real guitar tab + drum groove + a rendered chord chart; Leo's curated my-files; per-part Drums annotations) · **Amazing Grace** (PD, Newton 1779 — committed lead sheet + chord text chart). Setlist "Sat @ The Anchor" rebuilt on the three.
- **Charts:** extended `cmd/mkcharts` (pure Go / go-pdf, deterministic) with `house-rising-sun-tab.pdf` (Am–C–D–F–E arpeggio, generic demo arr.) + `house-rising-sun-drums.pdf` (6/8 groove box); plus `house-of-the-rising-sun.chart` + `amazing-grace.chart` (chord-dialect text charts). All original or public-domain — no copyrighted transcriptions.
- **Annotations:** new `buildBandChartAnnotations` places a light 3-layer set (mandatory conductor cues, shared section highlight, personal "My notes") in the *content band* of the committed charts — not the staff-relative/orchestral generic layout (I also fixed the `generated`/staff-placeholder flag to exclude committed `localPath` charts). `buildDrumPartAnnotations` gives the Drums part its own layer (per-file scoping).

**⚠ One deviation to flag:** the plan recommended **LilyPond** for the tab/drum charts; I used **`cmd/mkcharts` (Go)** instead — for **build/CI reproducibility** (LilyPond isn't in CI) and consistency with the existing committed charts (which mkcharts already generates, incl. the Open Road tab). The output is a clean, legible tab + groove box (pixel-checked). If you'd rather have LilyPond's richer engraving for the video, say so and I'll swap the two PDFs (committed artifacts either way).

**Verified (mine):** mkcharts renders both new charts clean + byte-deterministic; the seed runs **end-to-end** against a fresh mem instance (3 songs, real charts, text charts rendered server-side, all annotations imported, my-files/cues/setlist wired — full log clean); the **Studio viewer renders all three songs with clean layered annotations** (pixel-checked all three — screenshots in the session scratchpad: `vid-openroad2/house2/grace2.png`). `go test ./...` + gofmt + vet green; no `webassets/dist` churn.

**Not in this slice (later, per the plan's sequencing):** the ORCHESTRA rework (Mozart + Pachelbel — slice 2); the demo `.tstage` bundle regen + the title-referencing e2e/screenshot fixups (slice 3). The orchestra still seeds Mozart + Bach (unchanged, already PD). On GO I land `ee35f5f` + cite VLL 2026-08-04. — Web & Core Agent

## 2026-08-04 — DEMO-VID Part A slice 1 (band rework) GATE REVIEW (`ee35f5f`): GO TO LAND — copyright-safe real songs render clean with the full layer showcase

Verified `ee35f5f` (scratch worktree; seeded a fresh file-backed instance + Studio pixel-checked all three, my runs):
- **Copyright-safe [VERIFIED]:** the copyrighted band titles (Wonderwall/Hallelujah/Black Hole Sun, Oasis/Soundgarden/Cohen) are GONE from the seed. Band = **The Open Road** (original) + **House of the Rising Sun** (Traditional/PD) + **Amazing Grace** (Newton 1779/PD). The `.chart` sources are the genuine traditional lyrics + real chords (Am–C–D–F–E; G–G7–C–D–Em) — no copyrighted transcription. Orchestra untouched (slice 2), still Mozart+Bach (PD).
- **Real charts render [VERIFIED, pixels]:** House of the Rising Sun shows a real **guitar tab** (EADGBe arpeggio, chord shapes Am x02210 …, 6/8) + a Drums part; Amazing Grace a real lead-sheet chord chart (3/4); Open Road the lead sheet + tab. `mkcharts` re-render is byte-deterministic (no PDF diff).
- **Annotations placed nicely, all layers + conductor role [VERIFIED, pixels]:** Open Road keeps the bespoke flagship (green personal "capo 2 OK"+flourish, amber Chorus highlight+"everyone in", red mandatory "rit.—watch me" conductor cue, blue ellipses); House/Grace get a clean generic 3-layer set (red conductor cue, amber section, blue "breathe"/ellipse) in the content band. Per-file Drums layer scoping present.
- Seed runs end-to-end (3 songs, real charts, text charts rendered server-side, annotations/my-files/cues/setlist wired); `go test ./...` + gofmt + vet green; no `webassets/dist` churn.
- **LilyPond→mkcharts deviation: SIGNED OFF.** The lane used `cmd/mkcharts` (Go) instead of the plan's suggested LilyPond — the RIGHT call: LilyPond isn't in CI, mkcharts is deterministic + already owns the committed charts. No swap needed. (Plan said LilyPond "recommended where no PD PDF at hand" — reproducibility wins.)

Minor polish note for a later pass (non-blocking): House & Grace share identical generic annotation text/placement ("watch me — into the last verse", "breathe here", same ellipse spot) — a touch repetitive; vary in the slice-3 polish if convenient.
...
## 2026-08-04 — DEMO-VID Part A slice 2 (ORCHESTRA multi-part) BUILT — held at the gate (`33385d7`)

The orchestra's "real parts for each desk" showcase. Replaced the fetched Bach Air with **Pachelbel's Canon in D** (PD, 1680) as a MULTI-PART song — real engraved per-instrument parts (**Violin I / Viola / Cello**) rendered from committed **LilyPond** source (`docs/demo-charts/lilypond/canon-*.ly`). Cory (cello) curates a personal my-files view of just the cello part. One commit `33385d7` on `task/DEMOVID-A2-orchestra`, **not pushed**.

- **⚠ Toolchain decision to confirm: LilyPond (not mkcharts) for orchestral parts.** mkcharts is text/chord-only — it can't engrave a real staff. LilyPond produces professional notation (the plan sanctioned it for parts). It's a **dev-only** regeneration tool: run/CI use the committed PDFs (documented in the README; output may not be byte-identical across LilyPond versions — the only reproducibility caveat). Music is safe PD: Violin I = the standard Canon melody, Cello = the iconic ground bass (D–A–Bm–F#m–G–D–G–A).
- **Clean annotations (`buildCanonAnnotations`), overlap-scrutinized per your slice-1c note:** the LilyPond part has the staff at the top + whitespace below, so the conductor cue **rings the final bar** with "rit. — watch me" on a connector from the clear space below; a shared "**mf espr.**" dynamic; the cellist's personal "**Bowing**" layer (down-bow marks + dolce, correctly hidden from the admin's view). Nothing overlaps the engraving. New `downBow` helper.
- **Scope note:** the multi-part showcase is on **Canon in D**, not Eine kleine Nachtmusik (the storyboard eyed Eine kleine) — Canon is the more legible multi-part piece and I'm confident of the PD music. Eine kleine kept as a single fetched/generated piece (+ both in the Spring Concert setlist).
...
## 2026-08-04 — POST-LAND: DEMO-VID slice 2 `8cbc828` — CI GREEN (all five). **Slice 2 CLOSED.** Orchestra now a real multi-part showcase: Canon in D with LilyPond-engraved Violin I / Viola / Cello parts (committed, CI-safe) + overlap-clean conductor/dynamic annotations + the cellist's hidden personal Bowing layer. **Part A content is essentially done** (band slices 1/1b/1c + orchestra slice 2). Last: slice 3 (demo `.tstage` regen + title-referencing e2e/screenshot fixups), then Parts B/C/D recording.

## 2026-08-04 — DEMO-VID Part A slice 3 (bundle regen + copyright-title purge) BUILT — held at the gate (`00ce668`)

The last of Part A. Retire the copyrighted song *titles* everywhere they still surfaced, and re-bake the committed demo bundle from the new copyright-safe seed so the shipped `.tstage` matches the demo. One commit `00ce668` on `task/DEMOVID-A3-bundle-fixups` off `8cbc828`, **not pushed**.

- **`.tstage` RE-BAKED through the real pipeline [VERIFIED, pixels]:** seed → marie bakes "Sat @ The Anchor" → `POST …/bake` → download. Now **3 songs / 3 pages** — The Open Road (lead sheet, the flagship annotation showcase), House of the Rising Sun (guitar tab), Amazing Grace (lead sheet); default = lowest-DisplayOrder part per song. I composited the baked overlays over the baked raster and confirmed the flagship Open Road annotations survive the bake **cleanly**: green Capo-2 highlight + orange ⚠, gold chorus bracket, "everyone in!" margin label, the conductor cue ringing the final-chorus **G** with "rit. — watch me", the "drive" highlighter — true em-dashes throughout. `TestBundleGolden_RoundTrip` stays green (the new bundle.json is already canonical, so the mirror round-trips byte-identically).
- **Front-page/demo docs purged:** `README.md`, `docs/demo/README.md`, `docs/demo-charts/README.md` — prose now describes the real copyright-safe setlist (~356 KB, 3 songs), the stale "Why not the real songs?" section is reframed to "why original + public-domain only", and a regen-history entry is added. The only remaining "Wonderwall/…" strings are the *historical* provenance note + the new entry that has to name what was retired.
- **Studio screenshots regenerated from the new demo [mine to shoot]:** `studio-editor.png` (The Open Road with the redesigned annotations — a genuine upgrade over the old empty-staff "Black Hole Sun" with floating marks), `band-overview.png` (Songs 3), `setlist-cues.png` (the 3 new rows + Marie's mic + red-electric cues, House's Bm override). Alt-text updated. Same dimensions as the originals (playwright, phone/desktop viewports @2x).
- **⚠ Mobile-lane handoff (NOT touched):** `stage-page.png` / `stage-controls.png` are **Compose/Stage renders** and now stale — they still show "Black Hole Sun" from the *old* bundle. Re-shooting them from the new `demo-concert.tstage` is the **mobile agent's** job (I don't cross lanes). I de-titled their README alt-text in the meantime; the only remaining old-title *pixels* live in those two Stage PNGs, pending the mobile re-shoot. Flagging for the mobile handoff.
- **Test fixtures swept:** the self-contained e2e / Go / Kotlin fixtures that used the old titles as throwaway song names → vetted-safe names (Wonderwall → The Open Road, Hallelujah → Amazing Grace), distinctness preserved (SongDrawerTest keeps insertion order). Titles aren't copyrightable and these aren't shipped, but it leaves the project with zero copyrighted titles in code.
...
## 2026-08-04 — DEMO-VID Part A: Stage screenshots RE-SHOT + landed (`c1d9b0d`) — mobile dispatch fulfilled by web-core (VLL direct request)

VLL directly asked web-core to reshoot the two Stage screenshots (the mobile dispatch above), so I did them rather than leaving them for the mobile lane — **the dispatch is now fulfilled; mobile can skip it.** Re-shot `stage-page.png` + `stage-controls.png` from the new `demo-concert.tstage` on the Pixel_Tablet_Portrait AVD: imported the bundle, opened "Sat @ The Anchor" in TroubaStage, resolved identity via the P205 "Who are you?" picker (Marie), captured **The Open Road (song 1/3)** — immersive (letterboxed) + chrome-revealed. README alt-text updated to match the new pixels verbatim (no stale "♩=98"/"Black Hole Sun"). Same dims as originals (1200x1920). Landed `c1d9b0d` (Approved: Fable dispatch + VLL 2026-08-04); CI watched.

**Now zero copyrighted titles in shipped surfaces** (README + docs/demo + docs/screenshots). One leftover, flagged not touched: `docs/screenshots/tablet-stage-wonderwall.png` — an **unreferenced** mobile-lane QA proof (only a historical reviews.md line mentions it), still old content + old-title filename. I didn't create it (mobile lane) so I left it; **mobile may want to delete or re-shoot+rename it** to close the last old-title artifact. Part A content complete; Parts B/C/D (recording) next. — Web & Core Agent

## 2026-08-04 — POST-HOC VERIFY: mobile Stage re-shoot `c1d9b0d` — ✅ ACCURATE; the LAST old-title pixels are gone

The mobile lane re-shot `stage-page.png` + `stage-controls.png` from the new `.tstage` (docs/asset-only, landed to main). Verified doc↔pixel: both now show **The Open Road** (Song 1/3) as TroubaStage composites it — green Capo-2 highlight + ⚠, gold chorus bracket, "everyone in!" margin label, "drive" highlighter, chrome revealed (drawer/title/settings/pager/encore banner), "free to ship" footer — matching the updated alt-text; **zero "Black Hole Sun"** strings or pixels remain anywhere in the repo. Bonus: confirms the bake→Stage pipeline renders the new bundle's flagship annotations correctly end-to-end on the real app.

**The copyrighted-title purge is 100% complete** (code, docs, screenshots, Stage renders). DEMO-VID Part A fully done incl. the mobile follow-up. Remaining: Parts B (web recording) / C (app recording) / D (voiceover + assembly).

## 2026-08-05 — Layer audit (all demo songs) + CORE bake fileId-scoping fix — held at the gate (`2079ab8`)

VLL asked to audit the annotation layers of every track and correct them. I RENDERED each song's baked annotations (composited the bundle overlays over the baked raster) and inspected — found a real core bug plus two annotation issues. One commit `2079ab8` on `task/DEMOVID-layer-audit` off origin/main, **not pushed**.

- **⚠ CORE BUG FIXED — bake ignored per-file annotation scoping [VERIFIED, pixels].** The baker rendered EVERY layer of a song onto the baked default part regardless of each layer's `fileId`. On multi-file songs this bled another part's ink onto the wrong page: **House of the Rising Sun's Drums-part notes** ("snare — lay back", "keep it soft under the vocal") composited onto the baked **guitar tab**. Root cause: `snapshotToDoc(snap)` passed all `snap.Layers` to the overlay renderer; `b.eng.Head(songID)` returns the whole song (all files). Fix: `snapshotToDoc(snap, fileID)` keeps a layer only if `FileID == bakedFile.ID` (or empty = song-level/legacy). This is the through-the-bake half of B11/T40 per-file scoping, which was only enforced in the live editor. New unit test `TestSnapshotToDoc_FileScoping`. Re-baked: House now bakes **3 layers, not 4**; the guitar page is clean (pixel-verified).
- **Eine kleine Nachtmusik — wrong genre of annotations.** It used the generic `buildSongAnnotations`, whose **"Verse/Chorus/Bridge"** pop-song labels + title-box overlap are wrong for a classical Allegro (the fetched path even said **"Adagio"** — wrong tempo). New `buildEineKleineAnnotations`: **Allegro / Exposition / Development / dolce / molto rit.**, one-per-system, no title overlap (pixel-verified). NB it's still the generated **empty-staff placeholder** offline (the Mutopia fetch 404s) — the *annotations* are now correct; see the flag below.
- **Amazing Grace — orphan floating "everyone" label** (no connector, unclear — the exact defect from your slice-1c feedback). Replaced with a **"v.1 — softly"** dynamic anchored at the verse-1 bracket (pixel-verified).
- Open Road + Canon re-verified unchanged/clean.

**Verified (mine):** every song composited + eyeballed; House drum-bleed gone; `TestSnapshotToDoc_FileScoping` + bundle golden round-trip + `go test ./...` + gofmt green; no dist churn. Regenerated the committed `docs/demo/demo-concert.tstage` from the fixed baker; README sizes updated (~344 KB).
...
## 2026-08-06 — DEMO-VID layer-audit + real-editions STACK GATE REVIEW (`e9346d9`, 5 commits): GO TO LAND — with a non-blocking licensing note for Part D

VLL: "plan upgraded, continue." Reviewed the whole held stack (bake fix → annotation redesign+guitar → real orchestral editions → Eine kleine engraved → Greensleeves), verified at `e9346d9` (my runs):
- **CORE bake fileId-scoping fix [VERIFIED — real product bug]:** `snapshotToDoc(snap, fileID)` keeps only song-level (empty FileID) + the baked file's own layers; drops other files' layers + their objects. Exactly the B11/T40 per-file semantics carried through the bake (was live-editor-only). `TestSnapshotToDoc_FileScoping` covers keep-own / drop-other / keep-song-level. Fixes real cross-part ink bleed on multi-file bakes (Drums notes onto the guitar tab) — affects any band with multi-file songs, not just the demo.
- **Real professional editions throughout [VERIFIED, pixels]:** Eine kleine is now a genuine engraved Mozart edition (Violin I/II, Viola, Cello, Full score — 5 files; Allegro K.525) replacing the empty-staff placeholder; Greensleeves a real Mutopia voice+guitar edition (melody, 5 verses, tab); Canon a real published edition. The demo is now real notation end to end (+ Open Road original + House/Amazing purpose-built charts).
- **Annotations corrected + overlap-clean [VERIFIED, pixels]:** Eine kleine now classical (Allegro/crisp-attacks/leggiero, no title overlap — was wrongly Verse/Chorus + "Adagio"); Greensleeves margin cues clean; Amazing Grace orphan "everyone" → anchored "v.1 — softly" (lane pixel). All in clear margins.
- `go test ./...` + gofmt + bundle golden + scope test green; no dist churn; bundle now 4 songs/5 pages.

**LICENSING — attributed, substantially compliant; ONE non-blocking strengthening for Part D.** The demo now mixes PD (Open Road original, House/Amazing traditional, Mozart) + **CC-BY 4.0** (Canon, Fischer v. Mollard) + **CC-BY-SA 4.0** (Greensleeves, David Kastrup) — all attributed in `docs/demo-charts/README.md`, and author+source travel ON the rendered page (Kastrup + Mutopia watermark on Greensleeves) → into the raster/bundle/video. VLL chose Greensleeves knowing it's CC-BY-SA. **Verified gap (minor):** the `.tstage` bundle.json carries no attribution and the on-page credit lacks the explicit license NAME. **Not a blocker** (author+source travel; full license in README), but **Part D (video) MUST list the CC-BY / CC-BY-SA credits (name + license + link) in the video credits** — the video is a public distribution — and I recommend (non-blocking) a root-README/NOTICE pointer that demo-charts carry their own CC licenses. Logged so Part D can't forget.

GO — land `e9346d9` (Approved: trailer citing this verdict + VLL 2026-08-06) + reseed. Part A content is now "real editions" grade. Plan doc updated for the upgrade. Next: Parts B/C/D (recording) — Part D carries the license-credits requirement. — Fable (architect/reviewer)

## 2026-08-06 — POST-HOC VERIFY: NOTICE + root-README pointer (`4574855`) — ✅ licensing hygiene closed (my non-blocking rec, done)

The lane proactively landed the licensing recommendation from my stack verdict. Verified: top-level `NOTICE` correctly attributes both CC editions — Canon in D (© 2015 Michael Fischer v. Mollard, Mutopia, **CC-BY-4.0** + link) and Greensleeves (© 2014 David Kastrup, Mutopia, **CC-BY-SA-4.0** + link) — states the repo code is under LICENSE while the two charts REQUIRE attribution wherever redistributed, **explicitly names DEMO-VID Part D** as a redistribution that must carry it, and points to `docs/demo-charts/README.md` for full provenance; root README links the NOTICE. Attribution now lives at four levels (NOTICE / per-file README / on the rendered pages / the Part-D requirement) — thorough + compliant. Docs-only. The Part-D video-credits obligation remains tracked in the plan. Nothing outstanding on licensing.

## 2026-08-06 — POST-LAND: layer-audit + real-editions stack `8f617c5` — CI GREEN (all five, e2e incl.). **CLOSED.** Core bake fileId-scoping fix live (no cross-part annotation bleed on multi-file bakes — a real product fix, not just demo). Demo is now "real editions" grade throughout: engraved Mozart (multi-part + full score), real Canon (CC-BY) + Greensleeves (CC-BY-SA) editions, corrected clean annotations. NOTICE + repo attributions in place. **DEMO-VID Part A is complete + upgraded.** Remaining: Parts B (web recording) / C (app recording) / D (voiceover + assembly — carries the CC video-credits requirement).

## 2026-08-06 — POST-HOC VERIFY: screenshot refresh `bd41a43` (4-song band) — ✅ ACCURATE. setlist-cues shows the running order — Open Road (mic + red-electric cue), House of the Rising Sun (Bm override), Amazing Grace, Greensleeves — with per-member cues + bench section; band-overview reflects the 4 songs. Docs-only, doc↔pixel confirmed. Demo assets now fully match the upgraded 4-song / real-editions content.

## 2026-08-06 — DEMO-VID storyboard refreshed to the upgraded content (spec-maintenance, ready for Part B)

VLL: "continue." No Part B/C/D built yet; the most useful architect step before recording is making the storyboard the lane records against accurate to the shipped (upgraded) content — the scenes were drafted for the original 3-song / non-engraved version. Updated `docs/tasks/DEMO-VID-walkthrough-video-plan.md`:
- **Scene 11 setlist:** 3 songs → **4 songs** (Open Road, House of the Rising Sun, Amazing Grace, Greensleeves); + new **scene 10b** — a brief Greensleeves beat ("real published editions alongside your own charts").
- **Scene 13 orchestra:** now "**real engraved Mozart edition**, per-instrument parts + **full score**" (was generic "real parts") — matches the engraved multi-part reality.
...
## 2026-08-06 — DEMO-VID Part B FOUNDATION GATE REVIEW (`6030dd5`): GO TO LAND the foundation + PROCEED to the full walkthrough

Verified `6030dd5` (my runs): the walkthrough recorder runs green and produces a genuine **1920×1080** webm of the REAL seeded demo — I extracted a frame (ffprobe: 1920×1080) showing the 4-song Troubadours (Marie admin / Leo conductor / Sasha member; Amazing Grace, Greensleeves, House, Open Road). `script.md` is finalized + already carries the credits requirement (Canon CC-BY, Greensleeves CC-BY-SA) and the engraved-orchestra description (independent of but consistent with my storyboard refresh `3fd1e6c`). Clean tooling: dedicated `playwright.walkthrough.config.ts` (video:on, seeded backend via global-setup), the `.webm` gitignored (release artifact). tsc clean.

**Ruling on the three asks:**
1. **Narration tone — good.** Clear, warm, not overwrought ("The Open Road is the band's own song. Watch the annotations…"). Minor punch-ups optional, not needed.
2. **PROCEED to the full walkthrough — YES.** VLL's ask is "everything / how it works" starting from **create a band → invite → write a chart** (storyboard scenes 1–5) — the built-demo spine alone omits the creation flow he listed first. Build scenes 1–5 (register/create band, invite+roles, live text-chart create) + the live interactions (draw / **transpose** / **bake**) + **audio-first timing** (pace each dwell to its Piper TTS length — the actual "synced voiceover" requirement). Add the **Greensleeves band beat** (storyboard scene 10b) since it's a real edition worth 8s. The built-demo spine you have is the right backbone to extend.
