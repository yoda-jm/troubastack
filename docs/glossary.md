...
|---|---|---|
| **band** | the trusted unit of collaboration and the privacy boundary; every member receives all band data | `bandId` everywhere on main; the wire still spells it `group_id` on `Song`/`Setlist` (I1-frozen, commented at both ends — D2 closed) |
...
| **owner** | of a layer/object: `OwnerID` — **three spellings of "shared", one per surface** (D14): domain `_shared_` (`domain.SharedOwner`), baked `LayerImage.owner == ""`, `.tband` `owner: "_shared_"`; a personal owner is a member **id** in the domain and the baked bundle but a **username** in the `.tband`; of a rehearsal note: the caller, implicit | `owner != ""` IS the personal test at bake time (`viewfilter.go`) |
...
| **wet** | the in-progress stroke, before commit: a second canvas in Studio (`EditCanvas` in `EditCanvas.tsx`), a native overlay on the tablet | — |
...
| **variant** | the per-member concert `<setlistId>~<userId>` (B07) — **retired for MINTING, live for READING**: no new variants are made, and `ParseConcertID` still reads old ones so a member's pre-retirement concert still downloads (D17) | `?scope=mine`, "Bake my parts" |
...
| Term | Meaning | Surface names |
|---|---|---|
| **reading mode** | **Page \| Width \| Scroll**: whole page fitted / fit width, scrolls within the page / one continuous column | code `FitMode {FIT_PAGE, FIT_WIDTH, SCROLL}` (D4 closed: the docs say Page/Width/Scroll now) |
...
| **self-guarding fixture** | a test whose fixture is *asserted to be capable of failing*. T178's producer test orders its uuids to CONTRADICT drawing order, then asserts that they still do — so renaming them to agree fails with *"fixture is not discriminating"* rather than passing against a doc carrying nothing. Pairs with [discriminating vector]: that one makes the *input* able to fail, this one keeps it that way (2026-09-23) |
| **producer-side test** | when a consumer test proves a field is *honoured*, ask separately whether anything *emits* it. `web/bake`'s zorder test was green for months while core sent no `order` at all — it was handed one. Assert the CROSSING: what the producer emits, run through the consumer's own rule |
...
| D2 | ~~band vs group~~ | proto `group_id` on Song/Setlist, `band_id` on the bundle; `domain.Song.GroupID`; USER-JOURNEY said "groups" | **DONE 2026-09-23** — wire names kept (I1: renaming a published field buys a spelling and costs every deployed client), with a comment at **both** ends saying `group_id` IS the band id; the one prose instance fixed | web-core |
...
| D4 | ~~reading mode~~ | contract and app UI say Page / Width / Scroll; README and USER-JOURNEY said "fit modes", "fit page/width"; code `FitMode` | **DONE 2026-09-23** — both docs say Page / Width / Scroll. The `FitMode` enum is left alone: it is the app's internal name, not a word a musician reads | web-core |
...
| D7 | ~~role vocabulary~~ | USER-JOURNEY "admin/member"; design/04 "ADMIN/PERFORMER/CONDUCTOR"; proto and code "admin / conductor / member" | **DONE 2026-09-23** — both docs now say **membership role** `admin / conductor / member` (proto `bundle.proto:190` is the authority). Deliberately NOT swept: "a performer with a pedal", "performers get stable releases" and the like elsewhere are ordinary English for *someone performing*, not a claimed role name, and renaming them to "member" makes the sentences worse | web-core |
...
| D11 | ~~landmark labels~~ | the seven P206 landmarks showed their raw ids in the picker; `CUE_ICON_LABELS[id] ?? id` falls back silently | **DONE 2026-09-15** — labels added, and `glyph-labels.test.ts` enumerates the generated `GLYPH_IDS` so a new glyph is covered when it exists, not when somebody remembers | web-core |
...
| D14 | ~~shared-owner sentinel~~ | domain `OwnerID == "_shared_"`, baked `LayerImage.owner == ""`, export `owner: "_shared_"` | **DONE 2026-09-23 — documented, deliberately NOT unified** (Fable, ⟨GO⟩ `d36704c6`). The empty string is not an accident of serialisation: `bundle.proto:47` documents it and `viewfilter.go` **depends** on it — `owner != ""` is the personal test that decides what a musician sees on stage. Unifying means rewriting a visibility predicate and invalidating the encoding of every bundle already on a tablet, to remove an inconsistency that exists only between a domain sentinel and its wire form — which is what a serialisation boundary is for. The three-way mapping is in the **owner** row above, and `owner_forms_test.go` pins all three so a fourth cannot appear quietly | web-core |
| D15 | ~~stale Core comments~~ | four claims; **one** was real | **PARTLY DONE 2026-09-15.** `webapi.go` getMyFiles fixed — with no saved selection it returns the shared DEFAULT FILE, not the pool. The other three did **not** reproduce and were left alone: `sync/doc.go` says sync is "the hub AND the apply engine" and the apply path does live there (`sync.go`/`apply.go`/`mapping.go`) while `internal/engine` owns HEAD — both true; `TypeHighlight` carries no "replaced" comment at all; `KindLayerReorder` **is** mapped (`sync/mapping.go:64`). Changing correct text on the strength of a stale claim is the same defect as the drift itself | web-core |
| D16 | ~~WetCanvas vs EditCanvas~~ | the file was `WetCanvas.tsx` and exported `EditCanvas` | **DONE 2026-09-15** — file renamed to `EditCanvas.tsx`; the historical record (reviews, task specs) keeps the old name on purpose | web-core |
| D17 | ~~per-member variant~~ | declared retired in two places; `ParseConcertID`, the `~` separator and variant filtering are still live | **DONE 2026-09-23 — the row was wrong, not the code.** The feature IS retired (no new variants are minted) and the parser is **deliberate read-compat**, documented at `baker.go:273` with the scoping it serves at `:201-204`, so a member's pre-retirement concert still downloads. Deleting it would break exactly the users it was kept for, silently. Note the shape: **"the docs say retired but the code is live" is usually read-compat, not drift** (Fable, ⟨GO⟩ `d36704c6`) | web-core |
...
| D21 | ~~stale "Notes" comments~~ | re-scoped: two comments in `Viewer.tsx` named the rail tab "Notes" after it became "Annotations" (not five — the other matches described history correctly) | **DONE 2026-09-15** | web-core |
...
| D25 | ~~jump-mark spec header~~ | the header said the destination is placed first; the tool places the SOURCE first (`pendingJumpSrc`) | **DONE 2026-09-15** | web-core |
...
| D28 | auto-upload "armed" | the tablet's UI never says *arm* — VLL removed the verb (A77 ⟨D6⟩) because Studio owns **Arm live mode** and the two modes are mutually exclusive by construction; the code keeps `isArmed` / `armedUntil` / `ARMED_UPLOAD_WINDOW_MS` | **DELIBERATE — do not unify.** The collision is a UI problem; the internal concept is precise and tested. Same disposition as D14 (2026-09-24) | — |
