...
**📋 / ✅ tags (added 2026-08-25, updated 2026-08-26).** The prose below describes the repo **as audited on
the 25th** and is deliberately left as written; the tags record what happened after. **✅ = retired, with
the landing commit.** 📋 = filed and queued. Where a finding was only *half* fixed, the tag says which
half — an audit that overstates its own remediation is worse than one that never tracked it. A finding
tagged 📋 has a spec in
...
| # | Finding | Where |
|---|---|---|
| C1 ✅ **RETIRED** `6b54ebc` (T113) | **No LICENSE file** — `NOTICE` references one that doesn't exist; the code is legally "all rights reserved" while the repo is public with badges. | repo root |
...
> **RESOLVED 2026-08-28 by `6246c66`:** it does corrupt. Fable reproduced the pre-fix shape independently three
> times; every run **hung** on a racing `LinkedHashMap` resize rather than completing (the arbiter under the §3
> mutation was killed at 240 s vs **7.05 s** confined). Mobile reported a `ConcurrentModificationException` at
> iteration 0; Fable did not reproduce that specific symptom — same corruption, different manifestation.
>
...
- ✅ **RETIRED** `306d54b` (T110) — 27 vitest units in 1.4s; **the DOM half T110 scoped out is now closed too** ✅ **`d982106`** (T119 — jsdom component tests for both error boundaries' RENDER paths, the blank-page regression e2e can't reach; **34 tests / 7 files in 4.2s**). **[UPDATED 2026-08-27]** the `retries:0` half of this clause is superseded: T117 (`e596e4d`) made it `retries: CI ? 1 : 0` with a flaky-warning step; `workers:1` remains. Original finding: **Studio and ink have zero unit tests** — 713 lines of hit-testing geometry and the single authoritative renderer are validated only through a long serial browser suite (`workers:1`, `retries:0` — one flake reds the push; ~20 min on CI now). **📋 CORRECTION (2026-08-25, while reviewing T112).** This audit stated **in four places** (originally written as "twice" — the other two were found on 2026-08-27 and are corrected in place) that T93 removed the
  ...
  gone. **[UPDATED 2026-08-27]** the single-point-of-failure config is no longer unchanged: T117 (`e596e4d`) took `retries` to `CI ? 1 : 0` (a retried pass is surfaced, never a silent green) and added a ~60s `@smoke` branch-push gate; `workers:1` remains. The reviewer
...
- ⚠️ **PARTLY FIXED** — `internal/sync` covered ✅ `c3752d6` (T109); `:androidApp` test source set ✅ **`af21716`** (A47 — first suite on `sessionCookieFor`/`safeSegment`, both teeth-checked, wired into CI to fail on red); **iOS test execution, visual regression and coverage measurement are still OPEN**. Original finding: `internal/sync` (the WS hub) has **zero in-package tests**; `:androidApp` has no test source set (pure functions like `sessionCookieFor`, `safeSegment` untested); iOS tests compile but never execute; **no visual regression** anywhere; no coverage measurement.
...
**Frontend delivery:** ✅ **chunk RETIRED** `403016c` (T112) — `/login` 778→309 kB, measured on both commits. **The multi-line text divergence and the pointer-only editor in this paragraph are still OPEN.** Original finding: one **755 KB JS chunk + 1.34 MB pdf.js worker** downloaded by every visitor including `/login` — still zero code splitting on main (no `React.lazy`, no manualChunks) on a product meant to run off a garage laptop over Wi-Fi. Text annotation entry moved from `window.prompt()` to a proper in-app dialog on main (T90/T91/T101 — a real fix), but the dialog uses a single-line `<input>`, so the latent multi-line divergence remains: `textBBox` counts `\n` lines, ink's `drawText` still renders one `fillText` line. Editor is pointer-only — no arrow-nudge/Delete/Escape keyboard path.
...
**Hygiene & doc drift:** two git worktrees nested *inside* the repo (`gate-push/` — 88 commits ahead, `t93-wt/`) that are not gitignored and would be destroyed by `git clean -fd`; stray `a34f2/` (empty) and `core/<shortname>-8080.log`; an uncommitted `.gitignore` edit guarding real band data; `reviews.md` at 963 KB is the largest text file in the repo; `proto/README.md` still describes the buf-codegen mechanism that P203 explicitly closed; `risks.md` R4 keeps wording ARCHITECTURE amended; USER-JOURNEY lists P201/P205-S3 as open while README says landed; WebView leaks per Edit entry and exposes its JS bridge to any user-typed URL; `androidx.security-crypto` pinned to an alpha; `CORE_URL` default declared three times.
...
| Compose UI tests for Stage (pedal-after-sheet, blocked-turn cue, two-up, badge) + an `:androidApp` test source set | **M** | ⚠️ **HALF.** The `:androidApp` test source set **exists** (A47, extended by A48). Compose UI tests still do **not** — deliberately, and the cost is recorded: A49 and A50 ship policy-tested but **wiring-unguarded**, because asserting Compose effect wiring needs this rig. **A50's wiring was since confirmed ON DEVICE** (2026-08-28, DPAD injection) — confirmed once by hand, still **guarded by nothing**, so a regression would be silent. **[TAGGED 2026-08-28]** |
